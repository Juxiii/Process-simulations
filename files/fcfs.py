...
def fcfs(procesy):
    ...
    element = -1  # element kolejki wykonywany przez procesor
    iterator = 0  # licznik iteracji pojedynczego procesu
    temp = 0  # podreczna zmienna (zmienna do sprawdzania jedynie czekających elemntów w jednej z petli)
    ...
    # GLÓWNA PĘTLA - wykonuje się dopóki ostatni element nie będzie miał status "zakończony"
    while kolejka[-1].status != 3:
        ...
        if not procesor_pracuje:    # dodawanie procesu do pracy jezeli procesor jest wolny z listy procesow czekajacych
            while temp < zmiana_stat:  # iteracja po elementach które czekają
                if kolejka[temp].status == 1 and not procesor_pracuje:  # wybieranie tylko jednego procesu czekającego
                    kolejka[temp].status = 2    # ustawienie statusu procesu na "praca"
                    procesor_pracuje = True
                    element += 1                    # następny element z kolejki bedzie wykonywany
                    iterator = kolejka[temp].czas   # pobranie czasu procesu procesu do zmiennej
                temp += 1
            temp = 0
